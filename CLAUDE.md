...
GLM-5.3 call per source**. $0 infrastructure, no database, no local daemon.
...
- **Model:** `glm-5.3` locked (`SYNTHESIS_MODEL`); pricing in `pkm/llm/pricing.py`.
...
- Markdown-native `pkm lint` exists (broken `[[wikilinks]]` + review backlog;
  warn-only step in ingest.yml). A notes-count `dashboard` against `notes/`
  remains unbuilt (the old DB-backed one was retired).
